#!/usr/bin/env python3
"""
This module contains the different routes of the authentication blueprint
"""

import requests
from collections.abc import Mapping
from flask import json


def register_user(email: str, password: str) -> None:
    """
    testing register a new user endpoint
    """
    payload = {"email": email, "password": password}
    response = requests.post(f"{URL}/users", data=payload)
    assert response.status_code == 200


def log_in_wrong_password(email: str, password: str) -> None:
    """
    testing log in endpoint with wrong password
    """
    payload = {"email": email, "password": password}
    response = requests.post(f"{URL}/sessions", data=payload)
    assert response.status_code == 401


def log_in(email: str, password: str) -> str:
    """
    testing log in endpoint
    """
    payload = {"email": email, "password": password}
    response = requests.post(f"{URL}/sessions", data=payload)
    assert response.status_code == 200
    session_id = response.cookies.get("session_id")
    assert session_id is not None
    return session_id


def profile_unlogged() -> None:
    """
    testing profile endpoint when user is not logged in
    """
    response = requests.get(f"{URL}/profile")
    assert response.status_code == 403


def profile_logged(session_id: str) -> None:
    """
    testing profile endpoint when user is logged in
    """
    response = requests.get(
        f"{URL}/profile", cookies={"session_id": session_id})
    assert response.status_code == 200
    assert response.json() == {"email": EMAIL}


def log_out(session_id: str) -> None:
    """
    testing log out endpoint
    """
    response = requests.delete(
        f"{URL}/sessions", cookies={"session_id": session_id})
    assert response.status_code == 200
    assert response.json() == {"email": EMAIL}


def reset_password_token(email: str) -> str:
    """
    testing reset password token endpoint
    """
    payload = {"email": email}
    response = requests.post(f"{URL}/reset_password", data=payload)
    assert response.status_code == 200
    reset_token = response.json().get("reset_token")
    assert reset_token is not None
    return reset_token


def update_password(email: str, reset_token: str, new_password: str) -> None:
    """
    testing update password endpoint
    """
    payload = {"email": email, "reset_token": reset_token,
               "new_password": new_password}
    response = requests.put(f"{URL}/reset_password", data=payload)
    assert response.status_code == 200
    assert response.json() == {"email": EMAIL, "message": "Password updated"}


EMAIL = "guillaume@holberton.io"
PASSWD = "b4l0u"
NEW_PASSWD = "t4rt1fl3tt3"
URL = 'http://localhost:5000'


if __name__ == "__main__":

    register_user(EMAIL, PASSWD)
    log_in_wrong_password(EMAIL, NEW_PASSWD)
    profile_unlogged()
    session_id = log_in(EMAIL, PASSWD)
    profile_logged(session_id)
    log_out(session_id)
    reset_token = reset_password_token(EMAIL)
    update_password(EMAIL, reset_token, NEW_PASSWD)
    log_in(EMAIL, NEW_PASSWD)
